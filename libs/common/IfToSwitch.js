...
function handleIfStatement(path) {
    ...
    if (types.isIfStatement(consequent)) {
        if (types.isIfStatement(consequent.consequent)) {
            if (types.isBinaryExpression(consequent.consequent.test) && consequent.consequent.test.left.name === 'mi') {
        ...
        if (types.isBinaryExpression(consequent.body[0].test) && consequent.body[0].test.left.name === 'mi') {
    ...
    if (types.isIfStatement(alternate) ) {
        if (types.isBinaryExpression(alternate.test) && alternate.test.left.name === 'mi') {
        ...
        if (types.isBinaryExpression(alternate.body[0].test) && alternate.body[0].test.left.name === 'mi'){
...
function fix(path) {
    ...
    if (types.isIdentifier(node.discriminant) && node.discriminant.name === 'Ci') {
        ...
        for (var idx = 0; idx < node.cases.length; idx++) {
            ...
            if (c.consequent.length === 2) {
                ...
                if (types.isIfStatement(first) && types.isIdentifier(first.test.left) && first.test.left.name === 'mi') {
